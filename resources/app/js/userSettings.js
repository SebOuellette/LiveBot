...
function clearSettingsFile() {
    fs.writeFileSync("resources/app/json/logins.json", "");
}
...
let settings = {
    ...
    get settings() {
        let settings;
        try {
            settings = JSON.parse(fs.readFileSync("resources/app/json/logins.json"));
        } catch (e) {
    ...
    set settings(args) {
        ...
        fs.writeFileSync("resources/app/json/logins.json", JSON.stringify(settings));
    },
