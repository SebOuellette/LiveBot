  ...
  jsonObj.forEach((menuSection) => {
    ...
    menuSection.groups.forEach((group) => {
      ...
      category.onclick = () => {
        ...
        else category.animate(animations.flashTextRed, { duration: 350 });
      };
    });
  });
}
...
function genTokenBox(parent, placeholder, customClass) {
  ...
  document.getElementById('tokenbox').addEventListener('keydown', (event) => {
    if (event.keyCode === 13) {
      unloadAllScripts();
      setToken();
    }
...
function genShortInput(
  ...
  if (id == 'tokenbox') {
    input.addEventListener('keydown', (event) => {
      if (event.keyCode === 13) setToken(input.value);
    });
  }
}
      ...
      if (!option.classList.contains('disabled')) {
        ...
        if (!group.update) {
          eval(
            group.call.replace(
              'DROPDOWNS',
              `'${option.innerText.replace(/\\|/g, (c) => '\\' + c)}'`
            )
          );
        }
